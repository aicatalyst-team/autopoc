...
class TestListFiles:
    def test_lists_all_files(self, sample_tree: Path) -> None:
        result = list_files(path=str(sample_tree))
    ...
    def test_glob_pattern_filter(self, sample_tree: Path) -> None:
        result = list_files(path=str(sample_tree), pattern="**/*.py")
        assert "src/main.py" in result
        assert "src/utils.py" in result
    ...
    def test_empty_directory(self, tmp_path: Path) -> None:
        ...
        empty.mkdir()
        result = list_files(path=str(empty))
        assert "No files found" in result


class TestReadFile:
    def test_reads_file_contents(self, sample_tree: Path) -> None:
        result = read_file(path=str(sample_tree / "src" / "main.py"))
        assert "import os" in result
        assert "print('hello')" in result

    def test_nonexistent_file(self, sample_tree: Path) -> None:
        result = read_file(path=str(sample_tree / "nope.txt"))
    ...
    def test_truncates_large_file(self, tmp_path: Path) -> None:
        ...
        result = read_file(path=str(large))
...
class TestWriteFile:
    def test_writes_new_file(self, tmp_path: Path) -> None:
        ...
        result = write_file(path=target, content="hello world")
        assert "Successfully wrote" in result
        assert Path(target).read_text() == "hello world"

    def test_creates_parent_dirs(self, tmp_path: Path) -> None:
        target = str(tmp_path / "a" / "b" / "c" / "file.txt")
        result = write_file(path=target, content="nested")
        assert "Successfully wrote" in result
