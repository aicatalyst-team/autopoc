...
Optional environment variables for monthly mode:
- `AUTOPOC_MONTHLY_MODE` -- Set to "true" to enable monthly report mode
...
### Monthly Mode (Monthly Report Tab)
```bash
python -m autopoc.cli_tools sheet-reader \
  --sheet-id "$AUTOPOC_SHEET_ID" \
  --credentials "$AUTOPOC_SHEET_CREDENTIALS" \
...
**Standard Mode:** Pick the top `MAX_BATCHED_POC` (default 2) candidates by evaluation score.

**Monthly Mode:** Pick up to `MAX_MONTHLY_POCS` (default 5) approved unprocessed candidates.
